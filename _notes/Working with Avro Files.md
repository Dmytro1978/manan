...
This short article describes how to transfer data from a relational database to S3 and Hadoop using Apache Sqoop utitlity. The data will be stored in Avro data format. 

To transfer the data following technologies were considered:
- Apache Sqoop 1.4.7;
- Oracle 12c;
- Amazon EMR 5.16.0 (Hadoop distribution 2.8.4).

## Sqoop Command to Store Fata in Avro Format
...
sqoop import \
  ...
  --null-non-string '' 
```
Note that when you run the above command the target directory should not exist. Otherwise Sqoop command will fail.
...
If the data was transferred to HDFS you can use following command to retrieve the table schema:

```python
hadoop jar avro-tools-1.8.1.jar getschema /user/hive/warehouse/employee/part-m-00000.avro > employee.avsc
```
If the data was transferred to S3 you need to copy the Avro data file to local file system and then retrieve the schema:
...
## Creating Avro Table in Amazon Athena
...
## Working with Timestamp Data Type in Avro
...
### Hive
You create an Avro table in Hive as usually:
...
When you query the data, you just need to convert milliseconds to string:
```sql
from_unixtime(<Unix time column> div 1000)
```
Following sql-query does not use ***unix_timestamp()*** function and all timestamp values are shown in Unix time format: 
...
```
Next sql-query uses ***unix_timestamp()*** function and all timestamp values are shown in readable string format: 
```sql
hive> select 
    >     id, name, age, gen, 
...
```
### Athena
When you create an Athena table all ***long*** fields should be created as ***bigint*** in ***create table*** statement (not in Avro schema!):
```sql
    CREATE EXTERNAL TABLE employee
    (
      ID string,
...
## Storing Timestamp Values as Text 
